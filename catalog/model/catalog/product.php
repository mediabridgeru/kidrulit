...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
        ...
        if ($this->customer->isLogged() && !empty($query->row['customer_price'])) {
            $customer_prices_arr = unserialize($query->row['customer_price']);
            foreach($customer_prices_arr as $val) {
                if($val['customer_id'] == $this->customer->isLogged()) {
        ...
        if ($query->num_rows) {
			return array(
				...
				'sku'              => $query->row['sku'],
				'upc'              => $query->row['upc'],
                'upc_more'         => $query->row['upc_more'], // Имеются товары этой модели с другим ГТД
                'upc_quantity'     => $query->row['upc_quantity'], // Количество товара без группы
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
				'isbn'             => $query->row['isbn'],
				'mpn'              => $query->row['mpn'],
...
*/
    ...
    public function getProductOptionQuantity($product_id) {
        $option_quantity = 0;
                ...
                if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                    ...
                    foreach ($product_option_value_query->rows as $product_option_value) {
                        if (isset($product_option_value['quantity'])) {
                            $option_quantity += (int)$product_option_value['quantity'];
                        }
                    }
                }
            }
    ...
    public function getGroupProduct($model, $option) {
        $product = [];

        if (empty($option)) {
            $product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p WHERE p.model = '" . $model . "' AND p.upc_quantity > 0 ORDER BY p.upc, p.date_added");

            if ($product_query->num_rows) {
                $product = $product_query->row;
            }
        } else {
            $product_option_id = key($option);
            $product_option_value_id = $option[$product_option_id];

            $product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov WHERE pov.product_option_value_id = '" . (int)$product_option_value_id . "'");
            ...
            if ($product_option_value_query->num_rows) {
                ...
                if ($product_option_value_query->num_rows) {
                    $product = $product_option_value_query->row;
                }
            }
        }
