...
class YooMoneyPaymentKassa extends YooMoneyPaymentMethod
{
    ...
    public function __construct($config, $language = null)
    {
        ...
        $this->oauthToken                  = $this->config->get('yoomoney_kassa_access_token');
        $this->isTestShop                  = $this->config->get('yoomoney_kassa_test_shop');
        $this->isFiscalizationEnabled      = $this->config->get('yoomoney_kassa_fiscalization_enabled');
        $this->oauthTokenExpiresIn         = $this->config->get('yoomoney_kassa_token_expires_in');
        $this->oauthCmsState               = $this->config->get('yoomoney_kassa_oauth_state');
    ...
    {
        $client = $this->getYookassaClient($logger);
     ...
     * @return Client
     */
    public function getYookassaClient($logger = null)
    {
        if (!$this->yookassaClient) {
            $this->yookassaClient = new Client();
            $this->setClientAuth();
            $userAgent = $this->yookassaClient->getApiClient()->getUserAgent();
        ...
        if ($logger) {
            $this->yookassaClient->setLogger($logger);
        }
    ...
    private function setClientAuth()
    {
        if ($this->getOauthToken()) {
            $this->yookassaClient->setAuthToken($this->getOauthToken());
            return;
        }
