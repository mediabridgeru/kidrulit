<?php 
class ModelPaymentRpcod2ems extends Model {
  	...
  	public function getMethod($address, $total) 
	{
		...
		else
		{
			...
			{
				foreach($this->session->data['shipping_methods']['russianpost2']['quote'] as $key=>$val)
				{
					if( !empty($val['service_key']) && $val['service_key'] == 'ems_insured')
					{
						list($address['code1'], $address['code2']) = explode('.', $val['code']);
					}
				}
			}
			
			if( empty($address['code1']) )
			{
				$address['code1'] = 'any';
				$address['code2'] = 'any';
			}
