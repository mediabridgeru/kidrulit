<?php 
class ModelPaymentRpcod2 extends Model {
	...
	{
		$this->handleError("rpcod2-DEBUG: start");
		$this->load->model('shipping/russianpost2');
		
		if( !$this->config->get('rpcod2_status') )
		{
			$this->handleError("rpcod2-DEBUG: modul oplati otkluchon");
			return false;
		}
		...
		{
			$this->handleError("rpcod2-DEBUG: onlyshipping");
		...
		if( $this->config->get('rpcod2_geo_zone_id') )
		{
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('rpcod2_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
			
			if( !$query->num_rows ) {
				$this->handleError("rpcod2-DEBUG: filter po geo-zone");
				return false;
			}
		}
