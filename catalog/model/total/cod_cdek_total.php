...
class ModelTotalCodCdekTotal extends Model {
	...
	public function getTotal(&$total_data, &$total, &$taxes) {
		...
		if (!empty($this->session->data['payment_method']['code']) && $this->session->data['payment_method']['code'] == 'cod_cdek' && $this->cart->hasProducts()) {
				...
				switch ($price_info['mode']) {
					...
					case 'percent_total':
						...
						if (!empty($this->session->data['shipping_method'])) {
							$total_full += $this->session->data['shipping_method']['cost'];
						}

						$value = ($total_full / 100) * $value;
