<?phpclass ModelAccountCustomer extends Model {
        ...
        if (trim($email) != '') {
            if ($newsletter) {
                ...
                if ($query->num_rows) {
                    ...
                    $this->db->query("INSERT INTO " . DB_PREFIX . "contacts_unsubscribe SET send_id = '0', customer_id = '" . (int)$customer_id . "', email = '" . $this->db->escape(utf8_strtolower($email)) . "', date_added = NOW()");
    ...
    public function unsubscribe($email, $send_id = 0, $customer_id = 0) {
        if ($customer_id > 0) {
        ...
        if ($query->num_rows) {
            ...
            $this->db->query("INSERT INTO " . DB_PREFIX . "contacts_unsubscribe SET send_id = '" . (int)$send_id . "', customer_id = '" . (int)$customer_id . "', email = '" . $this->db->escape($email) . "', date_added = NOW()");
            $unsubscribe_id = $this->db->getLastId();
            $this->db->query("UPDATE " . DB_PREFIX . "contacts_newsletter SET unsubscribe_id = '" . (int)$unsubscribe_id . "' WHERE LCASE(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
        }
    }
