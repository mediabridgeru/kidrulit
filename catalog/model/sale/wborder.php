<?php
class ModelSaleWborder extends Model {
  ...
  public function editOrder($order_id, $data) {
    ...
    $statusrow = $this->db->query("SELECT `order_status_id` FROM `" . DB_PREFIX . "order` WHERE `order_id` = " . (int)$order_id)->row;
    ...
    if ($order_query->num_rows) {
      ...
      foreach($product_query->rows as $product) {
                if ($statusrow['order_status_id'] !== '9') { // Подвергаем обработке только не тронутые до этого товары в заказе с любым статусом, кроме "Отмена и аннулирование"
          ...
          if (isset($data['order_product'])) {
            foreach ($data['order_product'] as $order_product) {
              ...
              $order_product_id = $this->db->getLastId();
                ...
                if ($data['order_status_id'] !== '9') { // Подвергаем обработке только не тронутые до этого товары в заказе с любым статусом, кроме "Отмена и аннулирование"
                $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
              }
...
public function updateStatus($order_id, $status) {
  ...
  $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$status . "', notify = '1', date_added = NOW()");
  ...
  $order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$status . "' AND language_id = '" . (int)$order_info['language_id'] . "'");
...
public function addOrderHistory($order_id, $data) {
  ...
  if ($order_query->num_rows) {
    ...
    foreach($product_query->rows as $key => $product) {
      
        // Убирает со склада при статусе "Отмена и аннулирование" и добавляет на склад при смене с него на другие статусы
        if ($data['order_status_id'] == '9'  && $statusrow['order_status_id'] !== '9') { // Убирать (!) со склада, если с любого статуса статус сменился на "Отмена и аннулирование"
        $this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
        } elseif ($statusrow['order_status_id'] == '9' && $data['order_status_id'] !== '9') { // Добавлять (!) на склад, если статус "Отмена и аннулирование" сменился на любой другой
        $this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity - " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
      }
      ...
      foreach ($option_query->rows as $option) {
          ...
          // Повторяет верхние действия, но уже с опциями
        if ($data['order_status_id'] == '9'  && $statusrow['order_status_id'] !== '9') {
          $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
        } elseif ($statusrow['order_status_id'] == '9' && $data['order_status_id'] !== '9') {
          $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity - " . (int)$product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
        }
      }
    }
