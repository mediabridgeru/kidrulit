...
// added
            ...
            if(!empty($this->session->data['custom_customer_type_simple'])) {
                if($this->session->data['custom_customer_type_simple'] != 'individual') {
                    $template->data['do_hidesbrf_card'] = 1;
                }
            }
...
//
            ...
            if (in_array($order_info['payment_code'], $codes)) {
                ...
                $template->data['html_btn'] .= '<br /><div class="sbacquiring"><a href="'.$this->url->link('account/sbacquiring/change', 'code=' . $order_id . '&order_id=' . $order_id, 'SSL').'"><img src="https://kidrulit.ru/image/banners/card650.jpg"></a></div>';
            } else {
                $template->data['docs'] = 0;
                $template->data['invoice'] = '/';
                $template->data['torg12'] = '/';
                $template->data['delivery_act'] = '/';
                $template->data['html_invoice'] = '/';
                $template->data['html_torg12'] = '/';
                $template->data['html_btn'] = '';
            }
