...
class ModelToolAnyport extends Model {
	...
	public function exportToGoogleDrive(&$dataFiles, $rootFolders = array(), $folder = '', $backup = false, $exportType = 'Backup') {
		...
		if (filesize($filePath) <= $this->returnMaxAllowedMemory(52428800)) { //size is less or equal than 50 MB
			//Insert a file
	...
	/* ONEDRIVE FUNCTIONS */
	public function downloadFromOneDrive($file = '') {
			if (empty($file)) throw new Exception($this->language->get('anyport_no_file'));
			
			// Get access token
			$accessToken = $this->initOneDrive();
			
			//Get file
			$result = $this->oneDriveRequest('https://apis.live.net/v5.0/'.$file.'?access_token='.$accessToken);
	...
	public function listFromOneDrive($folder = '') {
		$accessToken = $this->initOneDrive();
		
		//GET FOLDER ID
		$folderId = $this->getOneDriveFolderId($folder, $accessToken);
		if ($folderId === false) throw new Exception('Could not locate the folder.');
		
		//Get files
		$result = $this->oneDriveRequest('https://apis.live.net/v5.0/'.$folderId.'/files?access_token='.$accessToken);
	...
	public function exportToOneDrive(&$dataFiles, $rootFolders = array(), $folder, $backup = false, $exportType = 'Backup') {
		$accessToken = $this->initOneDrive();
		...
		if (filesize($filePath) <= $this->returnMaxAllowedMemory(8388608)) { //size is less than 8 MB
			...
			$result = $this->oneDriveRequest('https://apis.live.net/v5.0/'.$folderId.'/files?access_token='.$accessToken, "POST", array('Content-Type: multipart/form-data; boundary=AaB03x'), $data_string);
		...
		if (!empty($result['id'])) {
			...
			$unlink = false;
			$filePath = ($this->config->get('config_secure') ? 'https://' : 'http://') . ANYPORT_SERVER_NAME . substr($filePath, strlen(ANYPORT_ROOT));
			$result = str_replace('{FOLDER_ID}', substr(urlencode($folderId), strrpos(urlencode($folderId), '.') + 1, strlen(urlencode($folderId)) - strrpos(urlencode($folderId), '.') - 1), $this->language->get('onedrive_redirect')).' You can download the generated file <a target="_blank" href="'.$filePath.'">HERE</a>';
		} else $result = false;
	...
	private function oneDriveRequest($url, $type = "GET", $header = array(), $data = '') {
	...
	private function initOneDrive() {
		// Get access token 
		$this->load->model('setting/setting');
		$authCode = $this->getSetting('AnyPortAuth');
		
		$authCode = !empty($authCode['oneDriveAuthCode']) ? $authCode['oneDriveAuthCode'] : NULL;
		...
		$clientSecret = (empty($setting['AnyPort']['Settings']['OneDrive']['ClientSecret'])) ? NULL : $setting['AnyPort']['Settings']['OneDrive']['ClientSecret'];
		...
		$accessToken = $this->oneDriveRequest('https://login.live.com/oauth20_token.srf', 'POST', array(                                                                          
	...
	private function getOneDriveFolderId($folder, &$accessToken) {
		if (!is_string($folder)) throw new Exception($this->language->get('anyport_folder_not_string'));
		//Find the folder ID
		$folder = $this->clearInvalidEntries(explode('/', $folder));
		//get root ID
		if (empty($folder)) {
			$result = $this->oneDriveRequest('https://apis.live.net/v5.0/me/onedrive?access_token='.$accessToken);
			if (empty($result['id'])) return false;
			$folderId = $result['id'];
		} else {
			$folderId = $this->getOneDriveFolderIdForMany($folder, $accessToken, 0);
		}
		return $folderId;
	}
	private function getOneDriveFolderIdForMany(&$folders, &$accessToken, $folderIndex, $url = NULL) { //init $folderIndex = 0
		$id = $url;
		$url = 'https://apis.live.net/v5.0/'.($url === NULL ? 'me/onedrive' : $url).'/files?access_token='.$accessToken;
		$folderId = false;
		if ($folderIndex < count($folders)) {
			$result = $this->oneDriveRequest($url);
			
			if (empty($result['data'])) return false;
			
			foreach ($result['data'] as $data) {
				if ($data['type'] != 'folder') continue;
				if (strcmp(mb_strtolower(($data['name']), 'UTF-8'), mb_strtolower(($folders[$folderIndex]), 'UTF-8')) == 0) $folderId = $this->getOneDriveFolderIdForMany($folders, $accessToken, $folderIndex + 1, $data['id']);
				else {
					continue;
				}
			}
	...
	public function exportRootFolders($rootFolders) {
		...
		switch ($zipEngine) {
			...
			case 'Tar' : {
				...
				$excludeCommands = array("--exclude='image/cache/*'", "--exclude='system/cache/*'", "--exclude='vqmod/vqcache/*'", "--exclude='image/imagecache/*'");
				...
				exec('cd ' . ANYPORT_ROOT . '; tar ' . implode(' ', $excludeCommands) . ' -cf ' . $tempdir . '/' . $zipfilename . ' ' . $inlineFiles);
			}
		}
	...
	public function createFinalArchive($files, $exportType = 'Backup') {
		...
		switch ($zipEngine) {
			...
			case 'Tar' : {
				...
				exec('cd ' . $tempDir . '; tar -cf ' . $tempDir.'/'.$zipfilename . ' ' . $inlineFiles);
			}
		}
	...
	public function restoreDatabaseFromFile($file) {
		...
		if ($info['extension'] == 'zip' || $info['extension'] == 'gz') {
			...
			if ($info['extension'] == 'zip') {
				...
				exec('cd ' . $decompressFolder . '; tar -xf ' . $file);
			}
		...
		if(count($valid_files)) {
			...
			switch ($zipEngine) {
				...
				case 'Tar' : {
					...
					exec('cd ' . dirname($destination) . '; tar -cf ' . basename($destination) . ' ' . $inlineFiles);
				}
			}
		...
		if (is_dir($rootPath.$dir)) {
			if ($dh = opendir($rootPath.$dir)) {
				...
				while (($file = readdir($dh)) !== false && $result) {
					if( ($file == ".") || ($file == "..") || ($dir == "image/cache") || ($dir == "system/cache") || ($dir == "vqmod/vqcache") || ($dir == "imagecache")) continue;
