		...
		<?php if (isset($coolfilter['coolfilters'])) { ?>
            ...
            <?php if ($coolfilter['style_id'] == 'checkbox') { ?>
                <div class="coolfilter-item coolfilter-item-checkbox">
                    ...
                    <?php foreach ($coolfilter['coolfilters'] as $coolfilter_value) { ?>
						<?php if ($coolfilter_value['count'] || !$count_enabled) { ?>
							<li><input type="checkbox" <?php if($coolfilter_value['active']) { ?>checked="checked"<?php } ?>><a href="<?php echo $coolfilter_value['href']; ?>" <?php if($coolfilter_value['active']) { ?>class="coolfilter_active"<?php } ?> data-key="<?php echo $coolfilter_value['key']; ?>" data-value="<?php echo $coolfilter_value['value']; ?>"><i></i><?php echo $coolfilter_value['name']; ?> <small><?php echo $coolfilter_value['view_count']; ?></small></a></li>
						<?php } else { ?>
...
if ($(".coolfilter_active").not( "#price" ).length > 0) {
	...
	var selected = '<div class="coolfilter-item coolfilter-item-checkbox coolfilter-selected" id="coolfilter-selected">';
	selected += '<a onclick="resetcoolfilter();"><?php echo $text_reset_coolfilter; ?></a><br>';	
	selected += '</div>';
