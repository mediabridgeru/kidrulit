  ...
  <div class="box-content">
    <div class="box-product">
      <?php foreach ($products as $key => $product) { ?><div class="col-xs-24 col-sm-8 col-md-6 col-lg-6" data-prodid="<?php echo $product['product_id']; ?>">
		...
		<div class="cart"><span class="btn btn-color"><input type="button" title="<?php echo $button_cart; ?>" value="<?php echo $button_cart; ?>" onclick="boss_addToCart('<?php echo $product['product_id']; ?>');" class="button"  /></span>
		</div><?php echo $product['sticker']; ?>
		</div>
        <?php } ?>
