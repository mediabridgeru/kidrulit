					...
					if (code != link.data('active')) {
						...
						$.getJSON("/index.php?route=shipping/cdek", data, function (json) {
							...
							if (json.status == 'ok') {
								...
								// added 
								$('#shipping_method_ajax').attr('value', info.get('address'));
								$('#shipping_pvz_ajax').attr('value', code);
								$(options.input).attr('data-shmeth', $(options.input).attr('data-subt') + ', ' + info.get('address')); // added
								//
