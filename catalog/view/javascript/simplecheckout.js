...
function simplecheckout_reload(from) {
    ...
    if (from === 'cart_value_decreased' || from === 'cart_value_increased' || from === 'cart_value_changed' || from === 'product_removed') {
        data += '&update_required=1';
    } else if (from === 'cart_changed') {
        data += '&update_done=1';
    }

    $('#checkout_customer_custom_customer_type:checked').unbind();
    $('input[name*="custom_inn"], input[name*="custom_bank_bic"]').unbind();
...
jQuery(function(){
    ...
    jQuery('input[reload][autocomplete]').live('change', function(){
        var from = jQuery(this).attr('reload');
        simplecheckout_reload.field = null;
        if (from.indexOf('checkout_') == 0) {
            simplecheckout_reload.field = jQuery(this).attr('id');
