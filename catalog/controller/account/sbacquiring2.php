...
class ControllerAccountsbacquiring2 extends ControllerAccountsbacquiring {
    private $error = [];

    public function index() {
        $this->load->model('account/sbacquiring2');

        $yu_codes = $this->model_account_sbacquiring2->getPayMethods();
        ...
        if (isset($this->request->get['code']) & isset($this->request->get['order_id'])) {
            ...
            $platp = substr($this->model_account_sbacquiring2->yanencrypt($order_info['order_id'], $this->config->get('config_encryption')) , 0, 8);
            ...
            $action = $this->url->link('account/sbacquiring2/pay', '', 'SSL');

            $this->data['merchant_url'] = $action . '&order_id=' . $inv_id . '&paymentType=' . $order_info['payment_code'] . '&code=' . substr($this->model_account_sbacquiring2->yanencrypt($order_info['order_id'], $this->config->get('config_encryption')) , 0, 8);

            $this->load->model('account/sbacquiring2');

            $paystat = $this->model_account_sbacquiring2->getPaymentStatus($inv_id);
            ...
            if ($paystat['status'] != 1) {
                ...
                if ($this->config->get($order_info['payment_code'] . '_hrefpage_text_attach')) {
                    ...
                    $this->data['hrefpage_text'] = $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($order_info['payment_code'] . '_hrefpage_text_' . $this->config->get('config_language_id')));
                }
                else {
            ...
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/sbacquiring2.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/account/sbacquiring2.tpl';
            }
            else {
                $this->template = 'default/template/account/sbacquiring2.tpl';
            }

            $this->children = ['common/column_left', 'common/column_right', 'common/content_top', 'common/content_bottom', 'common/footer', 'common/header', ];
    ...
    public function pay() {
        if (isset($this->request->get['code']) & isset($this->request->get['order_id'])) {
            ...
            if (in_array($this->request->get['paymentType'], $yu_codes)) {
                ...
                $password = htmlspecialchars_decode($this->model_account_sbacquiring2->yandecrypt($this->config->get($codeforpay . '_password') , $this->config->get('config_encryption')));

                if ($this->config->get($codeforpay . '_servadr') == 'real') {
                    $server = 'https://securepayments.sberbank.ru/payment/rest/';
                    $certname = 'sbcert.pem';
                }
                elseif ($this->config->get($codeforpay . '_servadr') == 'self') {
                ...
                if ($this->config->get($codeforpay . '_returnpage')) {
                    ...
                    $returnUrl = htmlspecialchars_decode($this->url->link('account/sbacquiring2/success') . '&payment_code=' . $codeforpay . '&ordernum=' . $order_info['order_id'] . '&code=' . $platp . $first);
                    $failUrl = htmlspecialchars_decode($this->url->link('account/sbacquiring2/fail') . '&payment_code=' . $codeforpay . '&ordernum=' . $order_info['order_id'] . '&code=' . $platp . $first);
                }
            ...
            if (!$this->config->get($codeforpay . '_cart')) {
                if ($order_info['email'] != '') {
                    if (!strpos($order_info['email'], '@localhost.net')) {
                        $okassa = ['customerDetails' => ['email' => $order_info['email'], ], 'cartItems' => ['items' => [], ], ];
                    }
                    else {
                        $okassa = ['customerDetails' => ['phone' => "+" . preg_replace('/[^0-9]/', '', $order_info['telephone']) , ], 'cartItems' => ['items' => [], ], ];
                    }
                }
                else {
                    $okassa = ['customerDetails' => ['phone' => "+" . preg_replace('/[^0-9]/', '', $order_info['telephone']) , ], 'cartItems' => ['items' => [], ], ];
                ...
                foreach ($vouchersbuy as $voucherbuy) {
                    $cart_products[] = ['quantity' => 1, 'name' => $voucherbuy['description'], 'price' => $voucherbuy['amount'], 'product_id' => 0, ];
                }
                ...
                foreach ($cart_products as $cart_product) {
                    ...
                    if (isset($customname)) {
                        $res = $this->model_account_sbacquiring2->getCustomName($cart_product['product_id'], $customname);
                    ...
                    $okassa['cartItems']['items'][] = ['positionId' => $posnum, 'name' => mb_substr(addslashes(stripslashes(str_replace("'", '', htmlspecialchars_decode($cart_product['name'])))) , 0, 100, 'UTF-8') , 'quantity' => ['value' => $cart_product['quantity'], 'measure' => 'шт', ], 'itemPrice' => $tovprice, 'itemCode' => $cart_product['model'] . $option_itemCode, 'tax' => ['taxType' => $ndsvalue, ], ];
                }
                ...
                if ($alldiscount == true) {
                    ...
                    if ($shipping >= 0 && $order_info['shipping_code'] != '') {
                        ...
                        $okassa['cartItems']['items'][] = ['positionId' => $posnum, 'name' => mb_substr(addslashes(stripslashes(str_replace("'", '', htmlspecialchars_decode($order_info['shipping_method'])))) , 0, 100, 'UTF-8') , 'quantity' => ['value' => 0, 'measure' => 'шт', ], 'itemPrice' => $shipping1, 'itemCode' => $order_info['shipping_code'], 'tax' => ['taxType' => 0, ], ];
                    }
                }
                ...
                if ($proverkacheck == - 0.01 || $proverkacheck == 0.01 || $proverkacheck == 0.02 || $proverkacheck == - 0.02 || $proverkacheck == - 0.03 || $proverkacheck == 0.03 || $proverkacheck == 0.04 || $proverkacheck == - 0.04 || $proverkacheck > 1 || $proverkacheck <= 1 && $proverkacheck != 0.00) {
                    ...
                    elseif ($proverkacheck == 0.01) {
                        $correctsum = 0.01;
                    }
                    elseif ($proverkacheck == 0.02) {
                        $correctsum = 0.02;
                    }
                    elseif ($proverkacheck == - 0.02) {
                        $correctsum = - 0.02;
                    }
                    elseif ($proverkacheck == - 0.03) {
                        $correctsum = - 0.03;
                    }
                    elseif ($proverkacheck == 0.03) {
                        $correctsum = 0.03;
                    }
                    elseif ($proverkacheck == 0.04) {
                        $correctsum = 0.04;
                    }
                    elseif ($proverkacheck == - 0.04) {
                        $correctsum = - 0.04;
                    }
                    elseif ($proverkacheck > 1) {
                        $correctsum = $proverkacheck;
                    }
                    elseif ($proverkacheck <= 1 && $proverkacheck != 0.00) {
                        $correctsum = $proverkacheck;
                    }
                    ...
                    if ($kopwar == false) {
                        foreach ($okassa['cartItems']['items'] as $item) {
                            if ($item['itemPrice'] > 0) {
                                ...
                                $posnum += 1;
                                $copyprod[] = ['positionId' => $posnum, 'name' => $okassa['cartItems']['items'][0]['name'], 'quantity' => ['value' => 1, 'measure' => 'шт', ], 'itemPrice' => $okassa['cartItems']['items'][0]['itemPrice'] + $correctsum, 'itemCode' => $okassa['cartItems']['items'][0]['itemCode'] . '~COPY', 'tax' => ['taxType' => $okassa['cartItems']['items'][0]['tax']['taxType'], ], ];
                ...
                if ($shipping >= 0 && $alldiscount == false && $order_info['shipping_code'] != '') {
                    ...
                    if ($shipping1 != '000') {
                        $okassa['cartItems']['items'][] = ['positionId' => $posnum, 'name' => mb_substr(addslashes(stripslashes(str_replace("'", '', htmlspecialchars_decode($order_info['shipping_method'])))) , 0, 100, 'UTF-8') , 'quantity' => ['value' => 1, 'measure' => 'шт', ], 'itemPrice' => $shipping1, 'itemCode' => $order_info['shipping_code'], 'tax' => ['taxType' => 0, ], ];
                    }
                }
                ...
                if ($this->config->get($codeforpay . '_debug')) {
                    ...
                    $ondsrules = [['id' => 0, 'name' => 'Без НДС', ], ['id' => 1, 'name' => 'НДС 0%', ], ['id' => 2, 'name' => 'НДС 10%', ], ['id' => 3, 'name' => 'НДС 18%', ], ['id' => 4, 'name' => 'НДС 10/110', ], ['id' => 5, 'name' => 'НДС 18/118', ], ];
            ...
            $postdata = http_build_query(['userName' => $userName, 'password' => $password, 'orderNumber' => $ordercompromis, 'amount' => $amount, 'returnUrl' => $returnUrl, 'failUrl' => $failUrl, 'description' => $description, 'currency' => $bankcurrency, 'orderBundle' => $okassacheck, 'language' => $language, 'jsonParams' => $jsonParams, ]);

            if ($this->config->get($codeforpay . '_zapros')) {
                $opts = ['http' => ['method' => 'POST', 'header' => 'Content-type: application/x-www-form-urlencoded', 'content' => $postdata, ], 'ssl' => ['verify_peer' => false, 'verify_peer_name' => false, ], ];
    ...
    public function callback() {
        if (isset($this->request->get['orderNumber']) && isset($this->request->get['operation'])) {
            ...
            $paystat = $this->model_account_sbacquiring2->getPaymentStatus($ordernum);
            ...
            if ($paystat['status'] != 1) {
                if ($this->request->get['operation'] == 'deposited' && $this->request->get['status'] == 1 && !$this->config->get($paymentcode . '_met') || $this->request->get['operation'] == 'approved' && $this->request->get['status'] == 1) {
                    ...
                    $password = htmlspecialchars_decode($this->model_account_sbacquiring2->yandecrypt($this->config->get($paymentcode . '_password') , $this->config->get('config_encryption')));

                    if ($this->config->get($paymentcode . '_servadr') == 'real') {
                        $server = 'https://securepayments.sberbank.ru/payment/rest/';
                        $certname = 'sbcert.pem';
                    }
                    elseif ($this->config->get($paymentcode . '_servadr') == 'self') {
                    ...
                    $postdata = http_build_query(['userName' => $userName, 'password' => $password, 'orderId' => $this->request->get['mdOrder'], 'language' => $language, ]);
                        ...
                        if ($payments) {
                            ...
                            if ($this->config->get($paymentcode . '_createorder_or_notcreate') && $order_info['order_status_id'] != $this->config->get($paymentcode . '_on_status_id')) {
                                ...
                                if ($this->config->get($paymentcode . '_mail_instruction_attach')) {
                                    ...
                                    $comment .= $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($paymentcode . '_mail_instruction_' . $this->config->get('config_language_id')));
                                ...
                                if ($this->config->get($paymentcode . '_success_alert_customer')) {
                                    if ($this->config->get($paymentcode . '_success_comment_attach')) {
                                        $message = $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($paymentcode . '_success_comment_' . $this->config->get('config_language_id')));
    ...
    public function success() {
        ...
        if (!$this->config->get('sbacquiring_callbackemulate')) {
            ...
            if ($paystat['status'] != 1) {
                if (isset($this->request->get['orderId'])) {
                    ...
                    $postdata = http_build_query(['userName' => $userName, 'password' => $password, 'orderId' => $this->request->get['orderId'], 'language' => $language, ]);
        ...
        if ($order_info['order_id']) {
            ...
            $action = $this->url->link('account/sbacquiring2');
            $online_url = $action . '&order_id=' . $order_info['order_id'] . '&code=' . substr($this->model_account_sbacquiring2->yanencrypt($order_info['order_id'], $this->config->get('config_encryption')) , 0, 8);
            ...
            if (true) {
                ...
                if ($this->config->get($paymentcode . '_success_page_text_attach')) {
                    $this->data['success_text'] .= $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($paymentcode . '_success_page_text_' . $this->config->get('config_language_id')));
                }
                else {
                ...
                if ($this->config->get($paymentcode . '_waiting_page_text_attach')) {
                    $this->data['success_text'] .= $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($paymentcode . '_waiting_page_text_' . $this->config->get('config_language_id')));
                }
                else {
            ...
            $this->data['breadcrumbs'][] = ['text' => $this->language->get('text_home') , 'href' => $this->url->link('common/home') , 'separator' => false, ];

            if (isset($first)) {
                $this->language->load('checkout/success');
                $this->data['breadcrumbs'][] = ['href' => $this->url->link('checkout/cart') , 'text' => $this->language->get('text_basket') , 'separator' => $this->language->get('text_separator') , ];
                $this->data['breadcrumbs'][] = ['href' => $this->url->link('checkout/checkout', '', 'SSL') , 'text' => $this->language->get('text_checkout') , 'separator' => $this->language->get('text_separator') , ];
                $this->data['button_ok_url'] = $this->url->link('common/home');
            }
            else {
                if ($this->customer->isLogged()) {
                    $this->data['breadcrumbs'][] = ['text' => $this->language->get('lich') , 'href' => $this->url->link('account/account', '', 'SSL') , 'separator' => $this->language->get('text_separator') , ];
                    $this->data['breadcrumbs'][] = ['text' => $this->language->get('history') , 'href' => $this->url->link('account/order', '', 'SSL') , 'separator' => $this->language->get('text_separator') , ];
    ...
    public function fail() {
        ...
        if ($this->request->get['ordernum'] == $order_info['order_id']) {
            ...
            if ($this->config->get($paymentcode . '_fail_page_text_attach')) {
                $this->data['fail_text'] .= $this->model_account_sbacquiring2->getCustomFields($order_info, $this->config->get($paymentcode . '_fail_page_text_' . $this->config->get('config_language_id')));
            }
            else {
                if ($order_info['order_status_id'] == $this->config->get($paymentcode . '_on_status_id')) {
