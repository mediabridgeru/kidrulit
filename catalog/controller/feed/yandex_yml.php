...
//---- Config section ----
	...
	public function index() {
		...
		if (!empty($this->request->get['id'])) {
			$customer_group_id = $this->request->get['id'];
		}

		$this->generateYml($customer_group_id);
		$this->response->addHeader('Content-Type: application/xml');
		$this->response->setOutput($this->getYml());
	}
	
	public function saveToFile($filename, $customer_group_id = 1) {
		$this->generateYml($customer_group_id);
		$fp = fopen($filename, 'w');
		$this->putYml($fp);
		fclose($fp);
	...
	protected function generateYml($customer_group_id = 1) {
		if ($this->config->get($this->CONFIG_PREFIX.'status')) {
			...
			foreach ($products as $product) {
				...
				$data['type'] = $product['manufacturer'] ? $datamodel : 'vendor.model'; //'vendor.model' или 'default';
...
//				$data['cbid'] = 15;
				...
				if (isset($yandex_yml_categ_delivery_cost[$product['category_id']]) && $yandex_yml_categ_delivery_cost[$product['category_id']]!='') {
					$data['local_delivery_cost'] = intval($yandex_yml_categ_delivery_cost[$product['category_id']]);
				}
				elseif ($local_delivery_cost!='') {
					$data['local_delivery_cost'] = intval($local_delivery_cost);
				}
				if ($pickup)
					$data['pickup'] = $pickup;
				if ($store)
...
//				$data['barcode'] = $product['sku'];
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		if (count($coptions)) {
			foreach ($coptions as $option) {
				...
				$option_price = $this->model_export_yandex_yml->getOptionPrice($product, $option, $data);

				$data_arr['price'] = $option_price['price'];

				if (isset($option_price['special'])) {
					$data_arr['oldprice'] = $option_price['price'];
					$data_arr['price'] = $option_price['special'];
				}
	...
	*/
	...
	protected function setOffer($data) {
		...
		$attributes = array('id', 'type', 'available', 'count', 'bid', 'cbid', 'param', 'group_id', 'accessory');
		...
		switch ($type) {
			...
			default:
				$allowed_tags = array_merge($allowed_tags, array('name'=>1, 'count'=>0, 'vendor'=>0, 'vendorCode'=>0));
				break;
		}
