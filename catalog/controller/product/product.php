...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		$this->data['isnotopt'] = ($this->customer->getCustomerGroupId() != 2);
		...
		if ($product_info) {
                ...
                if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
                    ...
                    foreach ($option['option_value'] as $option_value) {
                        if (isset($option_value['quantity'])) {
                            $option_set = true;
                            $option_quantity += (int)$option_value['quantity'];
                        }
            ...
            if ($option_set) { // если опции заданы
                if ($option_quantity <= 0) { // если сумма товаров в опциях < 0
                    $this->data['stock'] = $product_info['stock_status'];
                } elseif ($this->config->get('config_stock_display')) {
                    $this->data['stock'] = $option_quantity;
                } else {
			...
			$this->data['product_stickers'] = $this->getStickers($product_info['product_id']);
			...
			foreach ($results as $result) {
				if ($result['image']) {
					//$image = $this->model_tool_image->resize($result['image'], $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
					$image = $this->model_tool_image->resize($result['image'], 257, 257); // added
				} else {
					$image = false;
				}
