...
class ControllerPaymentYandexur extends Controller {
	protected function index() {
		...
		$this->language->load('payment/yandexur');
		$this->data['instructionat'] = $this->config->get('yandexur_instruction_attach');
		$this->data['btnlater'] = $this->config->get('yandexur_button_later');
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		$this->load->model('account/yandexur');
		$action = $this->url->link('account/yandexur/pay');
		$paymentredir = $action .
				'&paymentType=' . 'yandexur' .
		...
		$this->load->language('account/yandexur');

		if ($this->config->get('yandexur_fixen')) {
			if ($this->config->get('yandexur_fixen') == 'fix'){
			    $out_summ = $this->config->get('yandexur_fixen_amount');
			}
			else{
			    $out_summ = $order_info['total'] * $this->config->get('yandexur_fixen_amount') / 100;
			}
		}
		else{
		...
		if ($this->config->get('yandexur_createorder_or_notcreate')){
			if ($this->config->get('yandexur_otlog') == 'stock'){
		...
		else if ($this->config->get('yandexur_otlog') == 'pay'){
			$this->data['pay_url'] = $this->url->link('checkout/success');
		}
		...
		if ($this->config->get('yandexur_instruction_attach')){
			$this->data['text_instruction'] = $this->language->get('text_instruction');

			$instros = explode('$', ($this->config->get('yandexur_instruction_' . $this->config->get('config_language_id'))));
			$instroz = "";
			foreach ($instros as $instro) {
				if ($instro == 'href' || $instro == 'orderid' ||  $instro == 'itogo' || $instro == 'komis' || $instro == 'total-komis' || $instro == 'plus-komis'){
					...
					if ($instro == 'komis'){
						if($this->config->get('yandexur_komis')){
					    	$instro_other = $this->config->get('yandexur_komis') . '%';
						}
						else{$instro_other = '';}
					}
					if ($instro == 'total-komis'){
						if($this->config->get('yandexur_komis')){
					    	$instro_other = $this->currency->format($order_info['total'] * $this->config->get('yandexur_komis')/100, $order_info['currency_code'], $order_info['currency_value'], true);
						}
						else{$instro_other = '';}
					}
					if ($instro == 'plus-komis'){
						if($this->config->get('yandexur_komis')){
					    	$instro_other = $this->currency->format($order_info['total'] + ($order_info['total'] * $this->config->get('yandexur_komis')/100), $order_info['currency_code'], $order_info['currency_value'], true);
						}
						else{$instro_other = '';}
					}
				}
				...
				$this->data['yandexuri'] = $instroz;
		}
		
		
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/yandexur.tpl')) {
	      $this->template = $this->config->get('config_template') . '/template/payment/yandexur.tpl';
	    } else {
	      $this->template = 'default/template/payment/yandexur.tpl';
	    }
	...
	public function confirm() {
		...
		if ($this->config->get('yandexur_otlog') == 'stock'){
			if ($this->cart->hasStock()) {
				$ostatus = $this->config->get('yandexur_on_status_id');
				$comment = sprintf($this->language->get('stock'), $online_url);
			}
			else{
				$ostatus = $this->config->get('yandexur_start_status_id');
			...
			if ($this->config->get('yandexur_mail_instruction_attach')){
				
		    	$instros = explode('$', ($this->config->get('yandexur_mail_instruction_' . $this->config->get('config_language_id'))));
				      $instroz = "";
				      foreach ($instros as $instro) {
				      	if ($instro == 'href' || $instro == 'orderid' ||  $instro == 'itogo' || $instro == 'komis' || $instro == 'total-komis' || $instro == 'plus-komis'){
				      		if ($instro == 'href'){
