<?php
class ControllerModuleRecentViewProduct extends Controller
{
    private $error = array();
    ...
    public function index()
    {
        ...
        if ($product_info) {
            ...
            if ($product_info['image']) {
                ...
                if ($results) {
                    $this->data['popup'] = $this->model_tool_image->resize($results[0]['image'], $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
                    $this->data['thumb'] = $this->model_tool_image->resize($results[0]['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
                    unset($results[0]);
                } else {
                    $this->data['popup'] = '';
                    $this->data['thumb'] = '';
            ...
            $json = array();
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/recent_view_product.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/module/recent_view_product.tpl';
            } else {
                $this->template = 'default/template/module/recent_view_product.tpl';
            }
