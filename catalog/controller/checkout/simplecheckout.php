...
class ControllerCheckoutSimpleCheckout extends Controller
{
    public function index() {
        ...
        if (isset($this->request->post['update_done'])) {
            if ((int)$this->request->post['update_done']) {
                unset($this->session->data['update_required']);
            }
        } elseif (isset($this->request->post['update_required'])) {
            $this->session->data['update_required'] = $update_required = (int)$this->request->post['update_required'];
        } elseif (isset($this->session->data['update_required'])) {
            $update_required = (int)$this->session->data['update_required'];
        }

        $this->data['update_required'] = $update_required;
...
// added
        if (!empty($this->request->post['checkout_customer']) && !empty($this->request->post['checkout_customer']['custom_customer_type'])) {
            $this->session->data['custom_customer_type_simple'] = $this->request->post['checkout_customer']['custom_customer_type']; // mail
        }
...
//             
        if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
            ...
            $this->data['oferta'] = $this->url->link('information/information', 'information_id=3');            if (isset($this->request->post['acceptance'])) {                $this->data['acceptance'] = $this->request->post['acceptance'];            }            
            if (!isset($this->request->server['HTTP_X_REQUESTED_WITH']) || $this->request->server['HTTP_X_REQUESTED_WITH'] != 'XMLHttpRequest') {
    ...
    private function order() {
        ...
        if ($this->cart->hasShipping()) {
            ...
            if (isset($shipping_method['pvz'])) {
                $data['shipping_pvz'] = $shipping_method['pvz'];
            } else {
                $data['shipping_pvz'] = '';
            }
        } else {
        ...
        if ($version < 152) {
            ...
            if (!empty($shipping_method) && stripos($shipping_method['code'], "cdek") !== false) {
				$out = Array();
				$ttext = $shipping_method['text'];
				preg_match('/(\d+.?\d+)/',$ttext, $out);
				$ttext = $out[0];
				
                $this->db->query("UPDATE `" . DB_PREFIX . "order` SET cdek = '" . $this->db->escape($ttext) . "' WHERE order_id = '" . (int)$order_id . "'");
            }
        }
        
		
		$txt = '<!--';
		$txt .= print_r($this->session->data,true);
		$txt .= '\n\n\n';
		$txt .= print_r($ttext,true);
		$txt .= '\n\n\n';
