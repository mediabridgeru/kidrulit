...
class ControllerShippingRussianpost2 extends Controller {
	...
	public function index() 
	{
		...
		foreach($this->data['current_regions'] as $region ) 
		{
		...
		if (isset($this->request->post['russianpost2_if_nosrok'])) {
			...
			$this->data['russianpost2_if_nosrok'] = 'none';
		}
		...
		foreach($services as $service)
		{
			...
			/* start 1202 */
			if( $service['service_key'] == 'parcel_insured' )
			{
				
				$this->data['services_list'][] = array(
					"service_key" => 'split_parcel',
					"service_name" => $this->language->get('text_split_parcel_service_name'),
					"service_name_z" => $this->language->get('text_split_parcel_service_name'),
					"source" => isset( $russianpost2_services2api_list['parcel']['source'] ) ? $russianpost2_services2api_list['parcel']['source'] : ''
				);
				
				$this->data['services_list'][] = array(
					"service_key" => 'split_parcel_insured',
					"service_name" => $this->language->get('text_insured_split_parcel_service_name'),
					"service_name_z" => $this->language->get('text_insured_split_parcel_service_name'),
					"source" => isset( $russianpost2_services2api_list['parcel']['source'] ) ? $russianpost2_services2api_list['parcel']['source'] : ''
				);
			}
			/* end 1202 */
	...
	public function uploadt()
	{
		...
		$this->redirect($this->url->link('shipping/russianpost2', 'token=' . $this->session->data['token']) );
	}
