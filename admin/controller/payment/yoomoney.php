 ...
 * @property ModelPaymentYoomoneyOauth $model_payment_yoomoney_oauth
 *
...
class ControllerPaymentYoomoney extends Controller
{
    ...
    public function index()
    {
        ...
        $this->data['oauth_connect_url']        = htmlspecialchars_decode(
            $this->url->link('payment/yoomoney/oauthConnect', 'token='.$this->session->data['token'], 'SSL')
        );

        $this->data['oauth_token_url']          = htmlspecialchars_decode(
            $this->url->link('payment/yoomoney/oauthToken', 'token='.$this->session->data['token'], 'SSL')
        );

        $shopInfo = $this->getModel()->getShopInfo();

        $this->data['has_oauth_token']          = (bool)$kassa->getOauthToken();
        $this->data['is_test_shop']             = $shopInfo ? $shopInfo['test'] : null;
        $this->data['isConnectionFailed']       = empty($shopInfo);
    ...
    public function logs()
    {
        ...
        $this->data['breadcrumbs'] = array(
            array(
                'name' => $this->language->get('kassa_breadcrumbs_logs'),
                'link' => $this->url->link('payment/yoomoney/logs', 'token='.$this->session->data['token'], 'SSL'),
            ),
        );
    ...
    public function payments()
    {
        ...
        $kassa = $this->getModel()->getPaymentMethod(YooMoneyPaymentMethod::MODE_KASSA);
        if (!$kassa->isEnabled()) {
            $url = $this->url->link('payment/yoomoney', 'token='.$this->session->data['token'], 'SSL');
            $this->redirect($url);
        }
        ...
        $this->data['breadcrumbs']            = array(
            array(
                ...
                'link' => $this->url->link('payment/yoomoney/captureForm',
                    'token='.$this->session->data['token'].'&order_id='.$orderId, 'SSL'),
            ),
        );
    ...
    public function oauthConnect()
    {
        ...
        $this->load->model('payment/yoomoney_oauth');
        $response = $this->model_payment_yoomoney_oauth->getOauthConnectUrl();
    ...
    public function oauthToken()
    {
        ...
        $this->load->model('payment/yoomoney_oauth');
        $response = $this->model_payment_yoomoney_oauth->getOauthToken();
        $code = isset($response['error']) ? 502 : 200;
        $this->sendResponseJson(json_encode($response), $code);
    }

    /**
     * Устанавливает заголовки, кодирует ответ в формат JSON
     *
     * @param array $response
     * @param int $code
     * @return void
     */
    protected function sendResponseJson($response, $code = 200)
    {
...
'
    ...
    private function saveSettings($data)
    {
        ...
        $methodKassa = $this->getModel()->getPaymentMethod(YooMoneyPaymentMethod::MODE_KASSA);

        $settings['yoomoney_kassa_shop_id']                      = !empty($data['yoomoney_kassa_shop_id'])
                                                                        ? $data['yoomoney_kassa_shop_id']
                                                                        : $methodKassa->getShopId();
        $settings['yoomoney_kassa_access_token']                 = $methodKassa->getOauthToken();
        $settings['yoomoney_kassa_test_shop']                    = $methodKassa->isTestShop();
        $settings['yoomoney_kassa_fiscalization_enabled']        = $methodKassa->isFiscalisationEnabled();
        $settings['yoomoney_kassa_token_expires_in']             = $methodKassa->getOauthTokenExpiresIn();
        $settings['yoomoney_kassa_oauth_state']                  = $methodKassa->getOauthCmsState();
