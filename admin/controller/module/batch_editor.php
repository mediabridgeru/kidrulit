...
class ControllerModuleBatchEditor extends Controller {
	...
	public function index() {
		...
		$this->data['column_filters'] = 'Фильтры';
		...
		$lists = array ('manufacturer_id', 'stock_status_id', 'categories', 'customer_groups', 'tax_class_id', 'length_class_id', 'weight_class_id', 'discount_actions', 'calculate', 'stores', 'downloads', 'languages', 'attributes');
			...
			foreach ($this->data['setting']['fields'] as $name=>$field) {
				if ($name == 'image') {
					...
					if ($result['image'] && file_exists (DIR_IMAGE . $result['image'])) {
						$this->data['products'][$i]['thumb'] = $this->model_tool_image->resize($result['image'], 200, 200);
					} else {
	...
	public function getProductList() {
		...
		if (isset ($this->links[$path])) {
			...
			$this->data['product_image'] = $this->model_tool_image->resize($this->model_catalog_batch_editor_get_lists->getProductImage((int) $this->data['product_id']), 200, 200);
			...
			if ($results) {
				...
				foreach ($results as $result) {
					if (!is_dir ($result)) {
						...
						$images[] = array (
							'file' => $file,
							'img'  => $this->model_tool_image->resize($dir . $file, 200, 200)
						);
					}
