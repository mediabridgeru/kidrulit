...
class ControllerSaleCustomer extends Controller { 
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->get['c_type']) && isset($this->request->get['c_id'])){
				$custom_data = $this->model_tool_simplecustom->loadData($this->request->get['c_type'],$this->request->get['c_id']);
				//print_r($custom_data);
				
				$this->load->model('setting/setting');
				$store_id = $this->config->get('config_store_id');
				
				$simple_settings = $this->model_setting_setting->getSetting('simple',$store_id);
				$all_fields = $simple_settings['simple_fields_main'] + $simple_settings['simple_fields_custom'];
				$new_custom_data = array();
				foreach($all_fields as $key=>$field){
					if((substr($key, 0, 7) != 'custom_') || !(isset($this->request->post[$key])))
						continue;
					if(strlen($this->request->post[$key]) == 0)
						continue;
					$new_custom_data_field = array('id'=>$key,'label'=>$field['label']['ru']);
					if($field['type'] != 'text'){
						$values_texts = explode(';',$field['values']['ru']);
						foreach($values_texts as $value_text){
							
							if(!explode('=',$value_text)) continue;
							list($v, $t) = explode('=',$value_text);
							if($v == $this->request->post[$key]){
								$new_custom_data_field['value'] = $v;
								$new_custom_data_field['text'] = $t;
								break;
							}
						}
					} else {
						$new_custom_data_field['value'] = $this->request->post[$key];
						$new_custom_data_field['text'] = $this->request->post[$key];
					}
					
					if(!isset($new_custom_data_field['value']))
						continue;
					$new_custom_data[$key] = $new_custom_data_field;
				}
				$this->model_tool_simplecustom->setData($this->request->get['c_type'],$this->request->get['c_id'], $new_custom_data);
			}
