...
<div id="content">
  ...
  <?php } ?>
  <?php if ($error_stock) { ?>
  <div class="warning"><?php echo $error_stock; ?></div>
  <?php } ?>
  <div class="box" data-order_status_id="<?php echo $order_status_id; ?>">
    <div class="heading">
          ...
          <table class="list">
            ...
            <tbody id="product">
              ...
              <tr id="product-row<?php echo $product_row; ?>">
                ...
                <td class="left">
                  <?php if ( !$order_product['stock'] ) : ?><div style="color: red">***</div><?php endif; ?>
                ...
                <td class="right"><?php echo $order_product['quantity']; ?>
                  <input type="hidden" name="order_product[<?php echo $product_row; ?>][quantity]" value="<?php echo $order_product['quantity']; ?>" />
                    <?php echo $order_product['subtracted_products']; ?>
                    <input type="hidden" name="order_product[<?php echo $product_row; ?>][products]" value="<?php echo $order_product['products']; ?>" />
                  <?php if ( !$order_product['stock'] ) : ?><div style="color: red">На складе: <?php echo $order_product['db_quantity']; ?></div><?php else : ?><div style="color: grey">На складе: <?php echo $order_product['db_quantity']; ?></div><?php endif; ?>
                </td>
          ...
          <table class="list">
            ...
            <tbody>
              <tr>
                ...
                <td class="left">
<!-- added -->
                  <div id="shipping_method_ajax_wrap"><b>Способ доставки:</b> <?php echo $shipping_method; ?></div><br><br><a id="shipping_method_ajax_btn" onclick="reloadShippingMethod();" class="button">Обновить</a><br>
<!-- end //-->
<?php /*
...
*/ ?>
<!-- added -->
                  <input id="shipping_method_ajax" type="hidden" name="shipping_method" value="<?php echo $shipping_method; ?>" />
                  <input id="shipping_code_ajax" type="hidden" name="shipping_code" value="<?php echo $shipping_code; ?>" />
                  <input id="shipping_pvz_ajax" type="hidden" name="shipping_pvz" value="<?php echo $shipping_pvz; ?>" />
                  <input id="shipping_cdek_cost_ajax" type="hidden" name="shipping_cdek_cost" value="<?php echo $shipping_cdek_cost; ?>" />
<!-- end //-->
		...
		success: function(json) {
			...
			if (json['order_product'] != '') {
				...
				for (i = 0; i < json['order_product'].length; i++) {
					...
					/* Так как заказ может содержать дубликаты - проверяем и исключаем дублирование */
					var order_product_id = "";
					var product_id = product['product_id'];
					var model = product['model'];
					/* Line 75-77: Catalog/controller/checkout/manual.php */
					if (product['order_product_ids'][product_id + model] !== undefined) {
                        order_product_id = product['order_product_ids'][product_id + model];
                    }
                  ...
                  html += product['name'] + '<br /><input type="hidden" name="order_product[' + product_row + '][order_product_id]" value="' + order_product_id + '" /><input type="hidden" name="order_product[' + product_row + '][product_id]" value="' + product['product_id'] + '" /><input type="hidden" name="order_product[' + product_row + '][name]" value="' + product['name'] + '" />';
                  ...
                  html += 'На складе: ' + product['db_quantity'] + '</div>';
                  html += '</td>';
                    ...
                    if (i == 'cdek') {
                        ...
                        if (!json['shipping_method'][i]['error']) {
                            for (j in json['shipping_method'][i]['quote']) {
                                if (json['shipping_method'][i]['quote'][j]['code'] == $('input[name=\'shipping_code\']').attr('value') && json['shipping_method'][i]['quote'][j]['pvz'] == $('input[name=\'shipping_pvz\']').attr('value')) {
...
function reloadShippingMethod() {
  ...
  data += '#tab-total input[type=\'text\'], #tab-total input[type=\'hidden\'], #tab-total input[type=\'radio\']:checked, #tab-total input[type=\'checkbox\']:checked, #tab-total select, #tab-total textarea';
  $.ajax({
      url: '<?php echo $store_url; ?>index.php?route=checkout/shipping_method_ajax&token=<?php echo $token; ?>',
      type: 'post',
      data: $(data),
      dataType: 'html', 
      beforeSend: function() {
        $('#shipping_method_ajax_btn').after('<em><img src="view/image/loading.gif" alt="" /> <?php echo $text_wait; ?></em>');
      },      
      success: function(html) {
        $('#shipping_method_ajax_wrap').html(html);
        $('#shipping_code_ajax').remove();
        $('#shipping_method_ajax_btn').next('em').remove().next('b').remove();
      },
      error: function(xhr, ajaxOptions, thrownError) {
        alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
      }
...
*/
$('input[name=\'shipping_code\']').live('change', function(event) {
  var tgt = $(event.target);
  $('input[name=\'shipping_method\']').val($('input[name=\'shipping_code\']:checked').attr('data-shmeth'));

  $('#shipping_cdek_cost_ajax').val($('input[name=\'shipping_code\']:checked').attr('data-cdeckcost'));
});
</script>
<style type="text/css">
