...
<!-- Product No Sales -->
<!-- ------------ -->
<div class="tab-pane" id="product_nosales">
	<form class="form" 
		action="<?php echo $report_links['product_nosales']; ?>" method="post" 
		enctype="multipart/form-data" 
		id="form_product_nosales"
	>
		...
		<div class="well">
			<div class="row">
				<div class="col-sm-6">
				  ...
				  <div class="form-group">
				    <label class="control-label">
				    	<?php echo label($module_label, 'label_filter_onstore'); ?>
				    </label>
				    <?php echo echoSelect('IMReport[onstore]', $list_on_off, '1'); ?>
				  </div>
				...
				<div class="col-sm-3">
				  	<div class="form-group">
					    <label class="control-label" for="input-date-start">
					    	<?php echo label($module_label, 'label_filter_date_start_aval_product'); ?>
					    </label>
					    <div class="input-group date">
					      	<input type="text" name="IMReport[filter_date_start_aval_product]" 
					      		value="<?php echo date('d.m.Y'); ?>" 
					      		placeholder="<?php echo label($module_label, 'label_filter_date_start_aval_product'); ?>" 
				...
				<div class="col-sm-3">
				  	<div class="form-group">
					    <label class="control-label" for="input-date-end">
					    	<?php echo label($module_label, 'label_filter_date_end_aval_product'); ?>
					    </label>
					    <div class="input-group date">
					      	<input type="text" name="IMReport[filter_date_end_aval_product]" 
					      		value="<?php echo date('d.m.Y'); ?>" 
					      		placeholder="<?php echo label($module_label, 'label_filter_date_end_aval_product'); ?>" 
					      		data-date-format="DD.MM.YYYY" 
					      		class="form-control"/>
					      	<span class="input-group-btn">
					      	<button type="button" class="btn btn-default">
				...
				<div class="col-sm-12 report-btn-group">
					<button type="button"  
					  		class="btn btn-info pull-right button-rpns-6m">
						<i class="fa fa-calendar"></i> 
					  	<?php echo $module_button['button_rpns_6m']; ?>
					</button>
					<span class="pull-right">&nbsp;&nbsp;&nbsp;</span>
					<button type="button"  
					  		class="btn btn-info pull-right button-rpns-3m">
						<i class="fa fa-calendar"></i> 
					  	<?php echo $module_button['button_rpns_3m']; ?>
					</button>
					<span class="pull-right">&nbsp;&nbsp;&nbsp;</span>
					<button type="button"  
					  		class="btn btn-info pull-right button-rpns-1m">
						<i class="fa fa-calendar"></i> 
					  	<?php echo $module_button['button_rpns_1m']; ?>
					</button>
					<span class="pull-right">&nbsp;&nbsp;&nbsp;</span>
					<button type="button"  
					...
					<span id="save_status_product_nosales"></span>
				</div>
			</div>
		</div>
			...
			<table class="table table-bordered table-results">
				<thead>
					<tr>
						...
						<?php } ?>
						<th><?php echo label($module_table_header, 'table_product_nosales_name'); ?></th>
						<th><?php echo label($module_table_header, 'table_product_nosales_model'); ?></th>
						<th><?php echo label($module_table_header, 'table_product_nosales_manufact'); ?></th>
						<th><?php echo label($module_table_header, 'table_product_nosales_sku'); ?></th>
						<th><?php echo label($module_table_header, 'table_product_nosales_count'); ?></th>
						<th><?php echo label($module_table_header, 'table_product_nosales_price'); ?></th>
					</tr>	
				</thead>
				<tbody>
...
<script type="text/javascript">
	...
	function IMR_loadProductNoSales(form) {
		imrep.setTextStatus(form, {
			selector: '#save_status_product_nosales',
				...
				if (json['success']) {
					...
					for (var cnt = 0; cnt < json['data'].length; cnt++) {
						...
						row.html(
							...
							+ '</td>'
							+ '<td class="text-left">'
								+ item['sku'] //decodeURIComponent(item['model'])
							+ '</td>'
							+ '<td class="text-right">'
								+ item['count']
							+ '</td>'
							+ '<td class="text-right">'
								+ pattern.replace('[digit]', (new Number(item['price']).toFixed(2)))
							+ '</td>'
						);
					}
	...
	function IMR_loadProductNoSales_FakeLoad(form) {
		IMR_tuneUpTable(form, {
			colspan: 6 + (!!is_product_image_display),
			count: 0,
			cost: 0,
	...
	function IMR_loadProductNoSales_MonthsLoad(form, months)
	{
		var today = new Date(),
			prev = new Date()
		;
		...
		form.find('input[name="IMReport[filter_date_start]"]').val(IMR_toDateOnly(prev));
		form.find('input[name="IMReport[filter_date_end]"]').val(IMR_toDateOnly(today));
		
		form.find('input[name="IMReport[filter_date_start]"]').trigger('change');
		form.find('input[name="IMReport[filter_date_end]"]').trigger('change');
		
		form.find('.button-filter').trigger('click');
	}
	
	jQuery(function () {
		jQuery('#form_product_nosales .button-rpns-1m').click(function () {
			IMR_loadProductNoSales_MonthsLoad(
				jQuery(this).closest('form'),
				1
			);
		});

		jQuery('#form_product_nosales .button-rpns-3m').click(function () {
			IMR_loadProductNoSales_MonthsLoad(
				jQuery(this).closest('form'),
				3
			);
		...
		jQuery('#form_product_nosales .button-rpns-6m').click(function () {
			IMR_loadProductNoSales_MonthsLoad(
				jQuery(this).closest('form'),
				6
			);
