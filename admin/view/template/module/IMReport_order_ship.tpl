...
<!-- Order Shipping -->
<!-- ------------ -->
<div class="tab-pane" id="order_ship">
	<form class="form"
		action="<?php echo $report_links['order_ship']; ?>" method="post"
	...
	>
		...
		<div class="well">
			<div class="row">
				<div class="col-sm-6">
					...
					<div class="form-group">
						<label class="control-label">
							<?php echo label($module_label, 'label_filter_group_by'); ?>
						</label>
						<?php echo echoSelect('IMReport[group_by_time][]', $list_group_by_time, 'week', '', false); ?>
					</div>
				</div>
				<div class="col-sm-6">
					<!-- Shipping -->
				...
				<div class="col-sm-12 report-btn-group">
					...
					<span id="save_status_order_ship"></span>
				</div>
			</div>
		</div>
		<div class="table-responsive">
			<table class="table table-bordered table-results">
				<thead>
					<tr>
						<th><?php echo label($module_table_header, 'table_order_ship_date_start'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_date_end'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_code'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_method'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_title'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_count'); ?></th>
						<th><?php echo label($module_table_header, 'table_order_ship_cost'); ?></th>
					</tr>
				</thead>
				<tbody>
...
<script type="text/javascript">
	...
	var table_footer_all = "<?php echo label($module_table_header, 'table_footer_all');  ?>",
		previousPoint = null,
		previousLabel = null
	;

	function IMR_loadOrderShip(form) {
		imrep.setTextStatus(form, {
			selector: '#save_status_order_ship',
			...
			success: function (json) {
				if (json['success']) {
					...
					for (var cnt = 0; cnt < json['data'].length; cnt++) {
						...
						all_count += new Number(item['count']);

						row.html(
							'<td class="text-left">'
								+ IMR_toDateOnly(item['date_start'])
							+ '</td>'
							+ '<td class="text-left">'
								+ IMR_toDateOnly(item['date_end'])
							+ '</td>'
							+ '<td class="text-left">'
								+ item['code']
							+ '</td>'
