  ...
  <table id="discount_list" class="list">
   ...
   <tbody id="discount-row<?php echo $discount_row; ?>">
    <tr class="filter">
     ...
     <td class="left"><input type="text" name="product_discounts[<?php echo $discount_row; ?>][discount]" 
     value="<?php $product_percent = $product_discount['percent'];
      if ($product_percent > 0) {
        echo $product_discount['percent']; ?>" /> % 
            <select style="visibility:hidden;display:none;" name="product_discounts[<?php echo $discount_row; ?>][action]">
              <option value="equal_number"> = число</option><option value="minus_percent" selected> текущее значение - процент</option>
            </select>
     <?php  } else { 
        echo $product_discount['price']; ?>" /> 
            <select name="product_discounts[<?php echo $discount_row; ?>][action]">
              <option value="equal_number" selected> = число</option><option value="minus_percent"> текущее значение - процент</option>
            </select>
     <?php  } ?>
