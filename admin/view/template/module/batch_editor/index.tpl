...
function addDiscount(table) {
	...
	html += ' <input type="text" name="product_discounts[' + discount_row + '][discount]" value="" />  %  ';

  if (1) {
    html += '<select style="visibility:hidden;display:none;" name="product_discounts[' + discount_row + '][action]">';
    <?php foreach ($discount_actions as $discount_action) { ?>
    <?php   if ($discount_action["action"] != 'minus_number') { ?>  
      html += '<option value="<?php echo $discount_action["action"]; ?>" ';
    <?php if ($discount_action["action"] == 'minus_percent') { ?>
      html += 'selected';
    <?php } ?>
      html += ' ><?php echo $discount_action["name"]; ?></option>';
    <?php   } ?>
    <?php } ?>
    html += '</select>';
  }
