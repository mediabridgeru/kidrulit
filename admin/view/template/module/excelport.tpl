...
$(window).load(function() {
    ...
    var setLastImport = function(text) {
        ...
        if (typeof text == 'undefined') {
            text = $('#last_import').attr('data-text');
        }

        text = basename(text);

        if (text != '') {
            var value = $('#last_import').attr('data-template').replace('{FILE}', text);
            $('#last_import').html(value);
            $('#last_import_input').val(text);
            $('#last_import').show();
        }
    }
	...
	if (ajaxgenerate) {
		...
		var exportData = {
			ExcelPort : {
                ...
                LastImport : $('#last_import_input').val(),
				Settings : {
