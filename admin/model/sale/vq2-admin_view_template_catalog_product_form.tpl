...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-profile"><?php echo $tab_profile; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><a href="#tab-reward"><?php echo $tab_reward; ?></a><a href="#tab-design"><?php echo $tab_design; ?></a><a href="#tab-cprices">Customer prices</a></div>
...
<!-- end //-->
              ...
              <?php //print_r($product_option_value['pods']); ?>
...
<!-- added -->
                  <td class="left">
                    ...
                    <div id="customer<?php echo $option_row; ?><?php echo $option_value_row; ?><?php echo $pod_row; ?>" class="scrollbox pod-cutomers pod-cutomers<?php echo $pod_row; ?>" style="width:120px;height:80px;">

                    <?php $pod['customer_ids'] = isset($pod['customer_ids']) ? (is_array($pod['customer_ids']) ? $pod['customer_ids'] : unserialize($pod['customer_ids'])) : array(); ?>
<?php //print_r($pod['customer_ids']); ?>
                    <?php $this->load->model('sale/customer'); ?>

                    <?php foreach ($pod['customer_ids'] as $customer_id) { ?>

                    <?php $fln = $this->model_sale_customer->getCustomer($customer_id); ?>

                    <div class=""> <?php echo $fln['firstname'] . ' ' . $fln['firstname']; ?><img src="view/image/delete.png" />
                      <input type="hidden" name="product_option[<?php echo $option_row; ?>][product_option_value][<?php echo $option_value_row; ?>][pod][<?php echo $pod_row; ?>][customer_ids][]" value="<?php echo $customer_id; ?>" />
                    </div>
...
<!-- added -->
        ...
        <div id="tab-cprices" style="display:none">
          <table id="cprice" class="list">
            <thead>
              <tr>
            ...
            <?php $cprice_row = 0; ?>
            <?php foreach ($customer_prices as $customer_price): ?>
            <tbody id="cprice-row<?php echo $cprice_row; ?>">
              <tr>
                <td class="left">
                  <input type="text" name="customer_price[<?php echo $cprice_row; ?>][name]" value="<?php echo $customer_price['name']; ?>" />
                  <input type="hidden" name="customer_price[<?php echo $cprice_row; ?>][customer_id]" value="<?php echo $customer_price['customer_id']; ?>" />
                </td>
                <td><input type="text" name="customer_price[<?php echo $cprice_row; ?>][price]" value="<?php echo $customer_price['price'] ?>" /></td>
                <td class="left">
                  <a class="button" onclick="$('#cprice-row<?php echo $cprice_row ?>').remove()"><?php echo $button_remove ?></a>
                </td>
              </tr>
            </tbody>
            <?php $cprice_row++ ?>
            ...
            <tfoot>
              <tr>
                  <td <?php if ($this->config->get('pim_status') && !$this->config->get('pim_miu')) {?>colspan="3"<?php } else {?> colspan="2" <?php } ?>></td>
                  <td class="left"><a onclick="addcprice()" class="button">Добавить</a></td>
              </tr>
          </tfoot>
          </table>
        </div>
...
var cprice_row = <?php echo $cprice_row; ?>;

function priceautocomplete(cprice_row) {
  $('input[name=\'customer_price[' + cprice_row + '][name]\']').catcomplete({
    delay: 500,
    source: function(request, response) {
      $.ajax({
    ...
    select: function(event, ui) {
      $('input[name=\'customer_price[' + cprice_row + '][name]\']').attr('value', ui.item.label);
      $('input[name=\'customer_price[' + cprice_row + '][customer_id]\']').attr('value', ui.item.value);
...
//alert('asa');
    ...
    html  = '<tbody id="cprice-row' + cprice_row + '">';
    html += '<tr id="cprice-row' + cprice_row + '">';
    html += '<td><input type="text"  name="customer_price[' + cprice_row + '][name]" value="" /><input  type="hidden" name="customer_price[' + cprice_row + '][customer_id]" value="" /></td>';
    html += '<td><input type="text" name="customer_price[' + cprice_row + '][price]" value="" /></td>';
    html += '  <td class="left">';
    html += '    <a class="button" onclick="$(\'#cprice-row' + cprice_row + '\').remove()"><?php echo $button_remove ?></a>';
    html += '  </td>';
    html += '</tr>';
    html += '</tbody>';
    //alert(html);
    $('#cprice tfoot').before(html);

  priceautocomplete(cprice_row);
...
$('#cprice tbody').each(function(index, element) {
...
function addOptionValue(option_row) { 
  ...
  html += '  </tr>';
      ...
      html += '  <tr><td class="left"><?php echo $entry_info; ?></td><td colspan="9" class="left"><input name="product_option[' + option_row + '][product_option_value][' + option_value_row + '][ob_info]" value="" size="100"></td></tr>';
...
// added
          ...
          html += ' <div id="customer' + opt_row + ov_row+ ov_row +'" class="scrollbox pod-cutomers pod-cutomers' + pod_row + '" style="width:120px;height:80px;"><div>';
...
$('input[name=\'customers\']').live('keyup.catcomplete', function(){
  $(this).catcomplete({

  delay: 500,
  source: function(request, response) {
    var itgt = this.element.attr('data-nname');
    var tgt = this.element.attr('data-nnid');
    //alert(this.element.attr('data-nnid'));
    $.ajax({
      url: 'index.php?route=sale/customer/autocomplete&token=<?php echo $token; ?>&filter_name=' + encodeURIComponent(request.term),
      dataType: 'json',
      success: function(json) { 
  ...
  select: function(event, ui) {
    ...
    $('#customer' + ui.item.tgt).append('<div id="customer' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" alt="" /><input type="hidden" name="' + ui.item.itgt + '" value="' + ui.item.value + '" /></div>');
    $('.pod-cutomers div:odd').attr('class', 'odd');
    $('.pod-cutomers div:even').attr('class', 'even');
    return false;
  },
  focus: function(event, ui) {
        return false;
...
$('.pod-cutomers div img').live('click', function() {
  $(this).parent().remove();
  $('.pod-cutomers div:odd').attr('class', 'odd');
  $('.pod-cutomers div:even').attr('class', 'even');  
});
