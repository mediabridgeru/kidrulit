...
class BitbucketConnector
{
     ...
     * текущей версией. Для этого в репозитории должен быть файл latest/index.html c этой самой версией.
     *
    ...
    public function downloadLatestChangeLog($repository, $downloadDir, $fileName = 'CHANGELOG.md', $branch = 'master')
    {
        ...
        $options = array(
            CURLOPT_URL => $this->baseUrl . '/projects/' . $repository . '/raw/' . $fileName . '?at=' . urldecode('refs/heads/' . $branch),
    ...
    public function downloadRelease($repository, $version, $downloadDir, $type = self::ARCHIVE_BEST)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $this->baseUrl . '/rest/api/latest/projects/' . $repository . '/archive?at=' . urlencode('refs/tags/' . $version) . '&format=' . $ext,
            CURLOPT_RETURNTRANSFER => false,
            CURLOPT_FILE => $file,
