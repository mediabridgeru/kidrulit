		...
		if (in_array ($field, $product_description)) {
			...
			if (isset ($setting['fields'][$field])) {
				...
				if ($type[0] == 'varchar') {
					...
					$product_discounts = $this->db->query("SELECT product_id, price, percent FROM " . DB_PREFIX . "product_discount WHERE product_id IN (" . $product_id . ")")->rows; 
					if (!empty($product_discounts)) {
						foreach ($product_discounts as $key => $product_discount) {
							$new_discount_price = $value * (100 - $product_discount['percent']) / 100 ;
							$this->db->query('UPDATE ' . DB_PREFIX . 'product_discount SET price = ' . $new_discount_price . ', percent = ' . $product_discount['percent'] . ' WHERE product_id IN (' . $product_id . ')');	
						}	
					}
			...
			foreach($product_data_1 as $product_data) {
				foreach ($discounts as $discount) {
					if ($discount['action'] == 'equal_number') {
						...
						$product_data['percent'] = 100 * (1 - (float) $discount['discount'] / $product_data['price']) ;
