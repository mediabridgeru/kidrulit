...
class ModelToolAnyport extends Model {
	...
	private function initOneDrive() {
		...
		if (!empty($accessToken['access_token']) && !empty($accessToken['refresh_token'])) {
			$access_token = $accessToken['access_token'];
			// Save refresh token
			$token = $this->getSetting('AnyPortAuth');
			if (empty($token)) $token = array();
			$token['oneDriveRefreshToken'] = serialize($accessToken['refresh_token']);
			$this->editSetting('AnyPortAuth', $token);
		} else {
			$token = $this->getSetting('AnyPortAuth');
			if (empty($token['oneDriveRefreshToken'])) throw new Exception($this->language->get('anyport_need_authorize'));
			$refresh_token = unserialize($token['oneDriveRefreshToken']);
			$data_string = 'client_id='.$clientId.'&redirect_uri='.$redirectURI.'&client_secret='.$clientSecret.'&code='.$authCode.'&grant_type=refresh_token&refresh_token=' . $refresh_token;
			$accessToken = $this->oneDriveRequest('https://login.live.com/oauth20_token.srf', 'POST', array(
				'Content-Type: application/x-www-form-urlencoded',                                                                                
				'Content-Length: ' . strlen($data_string)                                                                      
			), $data_string);
	...
	private function getOneDriveFolderId($folder, &$accessToken) {
		...
		//get root ID
		if (empty($folder)) {
			$result = $this->oneDriveRequest('https://apis.live.net/v5.0/me/skydrive?access_token='.$accessToken);
