...
class ModelBatchEditorProduct extends Model {
	...
	public function getTotalProducts($data = array ()) {
		$query = $this->db->query("SELECT COUNT(DISTINCT p.`product_id`) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id AND pd.language_id = '" . (int) $data['language_id'] . "') WHERE p.product_id " . $this->getFilterSql());
		
		return $query->row['total'];
	}
	
	public function getProductId($data = array ()) {
		$sql = "SELECT DISTINCT p.`product_id` AS product_id FROM `" . DB_PREFIX . "product` p LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (p.product_id = pd.product_id AND pd.language_id = '" . (int) $data['language_id'] . "') WHERE p.product_id " . $this->getFilterSql();
		...
		foreach ($tables as $table => $fields) {
			...
			if ($sql_and) {
				if ($prefix) {
					...
					if ($table == 'url_alias') {
						$sql .= " AND p.`product_id` IN (SELECT REPLACE(query, 'product_id=', '') AS product_id FROM " . DB_PREFIX . $table . " WHERE " . implode (' AND ', $sql_and) . ")";
					} else {
						$sql .= " AND p.`product_id` IN (SELECT DISTINCT `product_id` FROM " . DB_PREFIX . $table . " WHERE " . implode (' AND ', $sql_and) . ")";
					}
				}
			}
			...
			if ($sql_or) {
				if ($prefix) {
					...
					if ($table == 'url_alias') {
						$sql .= " AND p.`product_id` IN (SELECT REPLACE(query, 'product_id=', '') AS product_id FROM " . DB_PREFIX . $table . " WHERE " . implode (' OR ', $sql_or) . ")";
					} else {
						$sql .= " AND p.`product_id` IN (SELECT DISTINCT `product_id` FROM " . DB_PREFIX . $table . " WHERE " . implode (' OR ', $sql_or) . ")";
					}
				}
			}
		}
