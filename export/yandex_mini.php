...
define('LOG_ERROR', dirname(__FILE__).'/log_err_mini.txt');
$t_start = microtime(1);
...
// товары
$sql = "SELECT p.`product_id`, p.`price`, p.`quantity`, p.`image`, pd.`name`, pd.`description` FROM `".DB_PREFIX."product` as p LEFT JOIN `".DB_PREFIX."product_description` as pd ON p.`product_id`=pd.`product_id` WHERE pd.`language_id`='{$language_id}' AND p.`price`>0 AND p.`quantity`>0 ORDER BY p.`product_id`";
$res = query($sql);
while($r = mysqli_fetch_assoc($res)) {
	...
	try {
		...
		$image = '';
		if (!empty($r['image'])) {
			if (file_exists(dirname(dirname(__FILE__)).'/image/'.preg_replace('/\s/', '%20', $r['image']))) {
				$image = $shop_url.'image/'.preg_replace('/\s/', '%20', $r['image']);
			}
		}
		
		if ($r['price']>0 && $r['quantity']>0 && !empty($descr) && !empty($image)) {
